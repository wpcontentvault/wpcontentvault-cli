...
class PurgeNotUsedAttachmentsCommand extends AbstractApplicationCommand
{
    /**
     ...
     * @var string
     */
    protected $signature = 'purge-not-used-attachments {--year=?}';
    ...
    public function handle(SitesRegistry $sites, ImageRepository $images): int
    {
        ...
        while ($hasNext) {
            ...
            $this->processAttachments($posts->posts, $images, $mainSite);
    ...
    private function processAttachments(array $attachments, ImageRepository $images, WPConnectorInterface $connector): void
    {
        foreach ($attachments as $attachment) {
            ...
            $connector->deleteAttachment($attachment->attachmentId);

            $this->info("Attachment {$attachment->attachmentId} deleted.");
        }
    }
}
