...
class TableRowRenderer implements BlockHtmlRendererInterface
{
    public function render(ObjectBlock $block, ChildBlockHtmlRendererInterface $childRenderer): string
    {
        assert($block->getType() === BlockTypeEnum::TABLE_ROW->value);

        return '<tr>'.$childRenderer->renderNodes($block->getChildren()).'</tr>';
    }
}
